...
const forgotPassword = async ({ email: email2 } = {}) => {
  ...
  return strapi.plugin("email").service("email").sendTemplatedEmail(
    ...
    {
      url,
      user: _$1.pick(user2, ["email", "firstname", "lastname", "username"])
    }
  ).catch((err) => {
...
const checkFieldsDontHaveDuplicates = (fields) => {
  ...
  return _$1.uniq(fields).length === fields.length;
};
...
const arrayOfConditionNames = yup.array().of(yup.string()).test("is-an-array-of-conditions", "is not a plugin name", function(value) {
  ...
  return _$1.isUndefined(value) || _$1.difference(value, ids).length === 0 ? true : this.createError({ path: this.path, message: `contains conditions that don't exist` });
});
const permissionsAreEquals = (a, b) => a.action === b.action && (a.subject === b.subject || _$1.isNil(a.subject) && _$1.isNil(b.subject));
...
const sanitizeUserRoles = (role2) => _$1.pick(role2, ["id", "name", "description", "code"]);
const sanitizeUser = (user2) => {
  return {
    ..._$1.omit(user2, ["password", "resetPasswordToken", "registrationToken", "roles"]),
...
const findRegistrationInfo = async (registrationToken) => {
  ...
  return _$1.pick(user2, ["email", "firstname", "lastname"]);
};
...
const update$3 = async (params, attributes) => {
  const sanitizedAttributes = _$1.omit(attributes, ["code"]);
  if (_$1.has(params, "id") && _$1.has(sanitizedAttributes, "name")) {
...
const hasSuperAdminRole = (user2) => {
  const roles2 = _$1.get(user2, "roles", []);
...
const init = () => {
  strapi.admin.services.passport.getPassportStrategies().forEach((strategy) => passport$1.use(strategy));
  registerAuthEvents();
...
const getTokenOptions = () => {
  ...
  return {
    secret,
    options: _$1.merge(defaultJwtOptions, options)
  };
};
const createToken = () => {
...
const mapKey = (key) => {
  if (_$1.isString(key) && key.startsWith("$") && key in operatorsMap) {
...
const unwrapDeep = (obj) => {
  if (!_$1.isPlainObject(obj) && !_$1.isArray(obj)) {
    return obj;
  }
  if (_$1.isArray(obj)) {
  ...
  return _$1.reduce(
    ...
    (acc, v, k) => {
      const key = mapKey(k);
      if (_$1.isPlainObject(v)) {
        if ("$elemMatch" in v) {
          _$1.setWith(acc, key, unwrapDeep(v.$elemMatch));
        } else {
          _$1.setWith(acc, key, unwrapDeep(v));
        }
      } else if (_$1.isArray(v)) {
        _$1.setWith(acc, key, v.map((v2) => unwrapDeep(v2)));
      } else {
        _$1.setWith(acc, key, v);
      }
      return acc;
    },
    {}
...
const index$4 = ({ ability, action: action2, model }) => ({
  ...
  getQuery(queryAction = action2) {
    if (_$1.isUndefined(queryAction)) {
...
const updateProjectSettings = yup.object({
  menuLogo: yup.string(),
  authLogo: yup.string()
}).noUnknown();
...
const updateProjectSettingsFiles = yup.object({
...
const validateUpdateProjectSettings = validateYupSchemaSync(updateProjectSettings);
const validateUpdateProjectSettingsFiles = validateYupSchemaSync(updateProjectSettingsFiles);
const validateUpdateProjectSettingsImagesDimensions = validateYupSchemaSync(
...
const admin = {
  ...
  async telemetryProperties(ctx) {
    ...
    const numberOfAllContentTypes = _$1.size(strapi.contentTypes);
    const numberOfComponents = _$1.size(strapi.components);
