...
const userDataSourceService = {
  getAll: async (keycloakSubject: string | undefined): Promise<StrapiServiceResponse<UserDataSource[]>> => {
    ...
    try {
      ...
      const response = await api.get(`/user-data-sources?populate[0]=dataSource&populate[1]=dataSource.image&filters[userProfile][keycloaksubject][$eq]=${keycloakSubject}`);
